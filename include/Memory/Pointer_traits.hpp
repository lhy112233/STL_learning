...
private:
  template <typename Tp, typename = void> struct element_type_impl;
  ...
  template <typename Tp, typename Up, typename = void> struct rebind_type {
    template <typename Tp_2, typename Up_2> struct replace_first_arg {};
...
public:
  ...
  template <typename U> using rebind = typename rebind_type<Ptr, U>::type;
