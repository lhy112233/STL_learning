...
namespace hy {
template <typename T, std::size_t N> struct array_traits_ {
  using type_ = T[N];
};

template <typename T> struct array_traits_<T, 0> {
  struct type_ {
    char unuse[1];
    constexpr operator T *() const noexcept { return nullptr; }
    constexpr T &operator[](std::size_t index) const noexcept {
      return reinterpret_cast<T &>(const_cast<char &>(unuse[index]));
    }
  };
};

template <typename T, std::size_t SIZE>
inline constexpr bool is_array_nothrow_swappable_v =
...
inline bool operator==(const hy::array<T, N> &lhs, const hy::array<T, N> &rhs) {
...
template <class T, std::size_t N>
inline bool operator<(const hy::array<T, N> &lhs, const hy::array<T, N> &rhs) {
  return std::lexicographical_compare(lhs.cbegin(), lhs.end(), rhs.cbegin(),
                                      rhs.cend());
}
